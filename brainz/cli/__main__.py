...
def get_users(host, port):
   ...
   """
    click.echo(get(f"http://{host}:{port}/users").json())
...
def get_user(host, port, user_id):
    ...
    """
    click.echo(get(f"http://{host}:{port}/users/{user_id}").json())
...
def get_snapshots(host, port, user_id):
    ...
    """
    click.echo(get(f"http://{host}:{port}/users/{user_id}/snapshots").json())
...
def get_snapshot(host, port, user_id, snapshot_id):
    ...
    """
    click.echo(get(f"http://{host}:{port}/users/{user_id}/snapshots/{snapshot_id}").json())
...
def get_result(host, port, save, user_id, snapshot_id, result):
    ...
    if save is not None:
        ...
        click.echo(get(f"http://{host}:{port}/users/{user_id}/snapshots/{snapshot_id}/{result}").json())
